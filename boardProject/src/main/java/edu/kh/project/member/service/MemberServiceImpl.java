...
//	- 팀 프로젝트, 유지 보수에 굉장히 도움이 많이되기 때문에! + AOP Proxy 적용을 위해서
...
public class MemberServiceImpl implements MemberService{
	 ...
	 * -> 입력된 문자열(비밀번호)에 salt를 추가한 후 암호화 -> 암호화 할 때 마다 결과가 다름
	 * -> DB에 입력 받은 비밀번호를 암호화해서 넘겨줘도 비교 불가능
	 * -> BCrypt가 함께 제공하는 평문, 암호화 데이터 비교 메서드인 "matches()" 이용하면 된다! (같으면 true, 다르면 false)
	 * 
	 * --> matches() 메서드는 자바에서 동작하는 메서드 -> DB에 저장된 암호화된 비밀번호를 조회해서 가져와야 한다!
	 */
...
//		log.debug("암호화 된 memberPw : {}", encorder.encode(memberPw));
