...
public class ExampleController {
	...
	// 1) page scope : 현재 페이지 내에서만 유효 해당 JSP 페이지에서만 접근 가능합니다
	
	// 2) request scope : 요청 받은 곳 + 요청 위임(forward) 받은 곳 (Servlet, JSP)
	
	// 3) session scope : 클라이언트가 서버 최초 연결 시 생성, 연결한 브라우저 종료 또는 세션 시간 만료까지
	
	// 4) application scope : 서버 실행 시 1개만 생성, 서버가 종료될 때 까지 유지
	 ...
	 */
							...
							// 주소 중 값을 얻어오고 싶은 부분을 {} 작성하고 {}를 지칭하는 이름(변수명) 내부에 작성
	@GetMapping("ex3/{number}")
	public String pathvariableTest(
			@PathVariable("number") int num
			// 주소 중 {number} 자리에 작성된 값을 얻어와 매개 변수 int num에 저장
