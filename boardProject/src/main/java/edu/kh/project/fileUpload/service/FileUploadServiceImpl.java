...
//	if(uploadFile.getOriginalFilename().equals(""))
		...
		String originalName = fileName.equals("") ? uploadFile.getOriginalFilename() : fileName + ext; // 입력된파일명.확장자 (3항 연산자)
	...
	public String test3(MultipartFile uploadFile) {
		...
		// 3) DB에 파일 정보 INSERT
		FileDto file = FileDto.builder()
				.fileOriginalName(uploadFile.getOriginalFilename()) // 원본 이름
				.fileRename(rename) // 변경 이름
				.filePath(testWebPath) // 웹에서 접근하는 경로
				.build();
...
//			throw new RuntimeException();
			...
			// 3) try-catch 작성 시 메서드 내부에서 예외가 처리되어 메서드 종료시 예외가 던져지지 않아
			...
			//	- @Transactional 어노테이션에 rollbackFor 속성 작성 안해도 롤백 처리 가능
			
			// 예외 강제 발생
			// - Unchecked Exception은 컴파일러가 자동으로 throws 구문을 작성 해줘서 예외 발생 시 호출부로 던져지게됨
