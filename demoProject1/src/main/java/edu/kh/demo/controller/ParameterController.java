...
public class ParameterController {
	...
	public String paramMain() {
		...
		// * Vuew Resolver(뷰 해결사)가 접두사 접미사를 붙여서 자동으로 forward를 진행
		// thymleaf의 접두사 : classpath:/templates/
		// thymleaf의 접미사 : .html
	...
	/* 1. HttpServletRequest.getParamete("key") 사용하기 */
...
//	@RequestMaPPing(value = "test1", method = Requestmethod.GET)
	...
	public String test1(HttpServletRequest req) {
		...
		// -> Controller 메서드에 작성된 매개 변수 타입에 맞는 객체가 존재하면 바인딩, 없으면 생성해서 바인딩
	...
	public String test2(
			...
			// -> HTML에서 얻어오는 모든 값은 기본적으로 String 형태이나 String이 매개 변수에 기입된 타입으로 자동으로 형변환/파싱을 수행해준다!!!!
	...
	public String test3(
			...
			// -> Map 형식으로 파라미터를 얻어오는 경우 == 특정 key만 얻어오는게 아닌 모든 파라미터를 얻어옴
	...
	public String test4(
		...
		/* type="text" 관련 input 태그는 값을 작성하지 않은 경우 빈칸(" ")이 제출 된다!!!
		 * -> 제출되는 값이 존재하기 때문에 @RequestParam()에 아무 속성을 적지 않아도 잘 얻어와 진다!!
		 * 
		 * * name 속성 == value 속성 (같은 것!!!)
		 * 
		 * * required : 필수 -> 기본 값이 true 
		 *  -> true가 설정된 경우 == 꼭 제출되어야 하는 파라미터
		 *  --> 만약 제출되지 않을 경우 HTTP 상태코드 400 -> 400 Bad Request 에러 발생
		 *  
		 * * checkbox, radio : 체크되지 않으면 제출 X -> 400 에러
		 * 
		 *  직접 쿼리스트링을 작성하는데 key=value가 누락된 경우 -> 400 에러
		 *  
		 *  required = false일 때 값이 제출되지 않으면 null
		 *  
		 *  * defaultValue : required = false 이면서, 값이 제출되지 않았을 경우 매개변수 대입할 값 지정
		 */
		...
		// 리다이렉트 하려는 요청 주소 앞에 "redirect:" 단어만 붙여주면 된다!!
	...
	public String test5(
			...
			// -> key값과 변수명이 같을 경우 원래는 @RequestParam의 ()생략이 가능했지만 지금은 생략하면 오류 발생!!!
