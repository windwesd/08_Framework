...
boardLike.addEventListener("click", e => {
  ...
  // 1. 로그인 여부 검사
  if(loginCheck === false){ // 로그인이 안되어있다
    alert("로그인 후 이용해 주세요");
    return;
  }
...
/* ** 삭제 버튼 클릭 시 **
  - 삭제 버튼 클릭 시 "정말 삭제하시겠습니까?" confirm()

  - /editBoard/delete, POST 방식, 동기식 요청
    -> form 태그 생성 + 게시글 번호가 세팅된 input
    -> body 태그 제일 아래 추가해서 submit()

  - 서버(Java)에서 로그인 한 회원의 회원 번호를 얻어와 로그인 한 회원이 작성한 글이 맞는지 SQL에서 검사
*/
...
/* * 수정 버튼 클릭 시 * 
  ...
  - 서버(Java)에서 로그인 한 회원의 회원 번호를 얻어와 로그인 한 회원이 작성한 글이 맞을 경우
    해당 글을 상세 조회 한 후 수정 화면으로 전환
*/
