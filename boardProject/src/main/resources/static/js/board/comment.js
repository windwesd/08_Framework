...
const selectCommentList = () => {
  ...
  // boardNo : 게시글 번호(boardDetail.js 전역 변수) (쿼리스트링은 무조건 GET 방식)
  ...
  .then(html => {
    ...
    // 타임리프가 해석된 html 코드를 .comment-list-area의 내용으로 대입 후 HTML 코드 해석
...
/** 답글 버튼 클릭 시 해당 댓글에 답글 작성 영억을 추가하는 함수
  @param btn : 클릭된 답글 버튼
*/
...
addComment.addEventListener("click", () => {
  ...
  // 2) 댓글 작성 여부 검사
  if(commentContent.value.trim().length === 0){ // 작성된 내용이 없다
    alert("내용을 작성해 주세요");
    return
  }
...
const addEventUpdateComment = () => {
  ...
  btns.forEach(btn => {
    btn.addEventListener("click", () => {
      showUpdateComment(btn); // 수정 버튼 클릭시 showUpdateCommet() 호출
    })
  })
}
