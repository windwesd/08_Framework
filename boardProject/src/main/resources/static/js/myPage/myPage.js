...
memberNickname?.addEventListener("input", () => {
  // input 이벤트 : 입력과 관련된 모든 동작 (JS를 이요한 값세팅 제외)

  // 입력된 값 얻어오기(양쪽 공백 제거 (trim))
  ...
  // 비동기로 입력된 닉네임 DB에 존재하는지 확인하는 Ajax 코드(fetch() API) 작성
...
// "변경하기" 버튼 클릭 시 또는 form 내부 엔터 입력 시 == submit(제출)

changePw?.addEventListener("submit", e => {

  // e.preventDefault(); // 기본 이벤트 막기 -> form의 기본 이벤트인 제출 막기
